...
if RATE_LIMITING_ENABLED:
    ...
    async def create_analysis(
        ...
        Rate limit configured via settings.rate_limit_per_minute
        """
    ...
    async def create_analysis(
        ...
        Args:
            ...
            confidence_threshold: Umbral de confianza (default from settings)
        ...
        _validate_file_format(file.filename)
        ...
        try:
            ...
            await _validate_file_size(image_data, settings.max_file_size)
...
async def create_public_analysis(
    ...
    try:
        ...
        # Call YOLO service with correct method name
...
async def list_analyses(
    ...
    # Set default limit from settings
    if limit is None:
...
def _build_heatmap_points(analysis: dict) -> List[dict]:
    """
...
async def get_heatmap_data(
    ...
    try:
        ...
        for analysis in result["data"]:
            points = _build_heatmap_points(analysis)
            for point in points:
...
async def get_map_statistics():
    ...
    try:
        ...
        # Estimate monitored area (configurable km² per unique location)
        area_monitored_km2 = round(len(unique_locations) * settings.area_per_location_km2, 1)
