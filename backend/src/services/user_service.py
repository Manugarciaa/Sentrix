    ...
    def create_user(db: Session, user_data: UserCreate) -> UserProfile:
        ...
        try:
            ...
            result = db.execute(text("""
                ...
                (id, email, hashed_password, display_name, organization, role, is_active, is_verified)
                VALUES (:id, :email, :hashed_password, :display_name, :organization, :role, :is_active, :is_verified)
                RETURNING id, email, display_name, organization, role, is_active, is_verified, created_at
            """), {
                "id": str(user_id),
            ...
            # Create UserProfile object from result
            row = result.fetchone()
            db_user = UserProfile(
                id=row[0],
                email=row[1],
