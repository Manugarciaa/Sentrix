...
class SupabaseManager:
    ...
    def _create_client(self) -> Client:
        ...
        # IMPORTANT: Use service role key for backend operations to bypass RLS
        # The anon key respects RLS policies, but backend needs admin access
        supabase_key = self._settings.supabase_service_role_key or self._settings.supabase_key

        if self._settings.supabase_service_role_key:
            logger.info("using service role key for backend operations (RLS bypass)")
        else:
            logger.warning("service role key not found, using anon key (RLS will be enforced)")

        # Check if we're in testing mode, development mode, or using mock URLs
        is_testing = os.getenv('TESTING_MODE', 'false').lower() == 'true'
        is_development = os.getenv('ENVIRONMENT', 'development').lower() == 'development'
        is_mock = not supabase_url or not supabase_key or 'mock' in supabase_url.lower()

        # In development or testing mode with mock values, create a mock client
        if is_mock and (is_testing or is_development):
        ...
        # In production with mock values, raise an error
        if is_mock:
            raise ValueError(
        ...
        try:
            ...
            if is_testing or is_development:
                # In testing/development, return a mock instead of failing
                logger.warning("failed to create Supabase client, using mock", error=str(e))
                from unittest.mock import MagicMock
                mock_client = MagicMock()
                mock_client.table = MagicMock(return_value=MagicMock())
                return mock_client
    ...
    def upload_image(self, image_data: Union[bytes, BytesIO], filename: str, bucket_name: str = None) -> dict:
        """
        ...
        try:
            ...
            # If upload succeeds, response will contain the path
            # If it fails, an exception is raised
            # Get public URL
            public_url = self.client.storage.from_(bucket_name).get_public_url(unique_filename)

            # Clean up URL - remove trailing ? if present (bug in some Supabase client versions)
            if public_url:
    ...
    def upload_dual_images(self, original_data: bytes, processed_data: bytes, base_filename: str) -> dict:
        ...
        """
        try:
            # Upload original image to sentrix-images bucket
            ...
            # Upload processed image to sentrix-processed bucket
            processed_result = self.upload_image(
                processed_data,
                f"processed_{base_filename}",
                self._settings.supabase_processed_bucket
            )
