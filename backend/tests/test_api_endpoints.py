...
class TestHealthEndpoints:
    ...
    def test_yolo_health_endpoint_success(self, client: TestClient, monkeypatch):
        """Test YOLO health endpoint with successful connection"""
        # Mock a successful health check
        async def mock_success_health():
            return {
                "healthy": True,
                "response_time_ms": 50,
        ...
        monkeypatch.setattr("src.api.v1.health.check_yolo_service", mock_success_health)
        ...
        assert "timestamp" in data
        assert data["yolo_service"]["healthy"] == True
    ...
    def test_yolo_health_endpoint_failure(self, client: TestClient, monkeypatch):
        ...
        monkeypatch.setattr("src.api.v1.health.check_yolo_service", mock_failed_health)
...
class TestAnalysisEndpoints:
    ...
    def test_create_analysis_with_file(self, client: TestClient, sample_image_data):
        ...
        # Patch the analysis service's process_image_analysis method
        with patch('src.services.analysis_service.analysis_service.process_image_analysis',
            ...
            assert "ms" in result["estimated_processing_time"]  # Should be like "1234ms"
