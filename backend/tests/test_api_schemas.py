...
def test_settings_configuration():
    ...
    try:
        ...
        print(f"   URL de base de datos configurada: {'Sí' if settings.database_url else 'No'}")
        print(f"   Host de API: {settings.backend_host}:{settings.backend_port}")
        print(f"   URL del servicio YOLO: {settings.yolo_service_url}")
        return True
...
def test_core_services_import():
    ...
    try:
        from ..core.services.yolo_service import YOLOServiceClient as CoreYOLOServiceClient
...
def test_schema_validation():
    ...
    try:
        from ..schemas.analyses import AnalysisUploadRequest, LocationData
