...
def test_on_circuit_open_callback():
    ...
    with patch('src.core.services.yolo_service.logger') as mock_logger:
        ...
        assert "yolo_circuit_breaker_opened" in call_args[0]
...
def test_on_circuit_close_callback():
    ...
    with patch('src.core.services.yolo_service.logger') as mock_logger:
        ...
        assert "yolo_circuit_breaker_closed" in call_args[0]
...
def test_on_circuit_half_open_callback():
    ...
    with patch('src.core.services.yolo_service.logger') as mock_logger:
        _on_circuit_half_open(mock_breaker)

        mock_logger.info.assert_called_once()
        call_args = mock_logger.info.call_args
        assert "yolo_circuit_breaker_half_open" in call_args[0]
