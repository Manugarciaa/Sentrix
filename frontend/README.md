...
graph TB
    ...
    style Z fill:#443e38,stroke:#333,color:#fff
    style TQ fill:#ff4154,stroke:#333,color:#fff
    style AC fill:#5a29e4,stroke:#333,color:#fff
    style BE fill:#4a9eff,stroke:#333,color:#fff
```
...
graph TB
    ...
    HM-->CRD
    RB-->BDG
    style AL fill:#4a9eff,stroke:#333,color:#fff
    style DP fill:#66bb6a,stroke:#333,color:#fff
    style AC fill:#ff9800,stroke:#333,color:#fff
    style BTN fill:#9c27b0,stroke:#333,color:#fff
```
...
│   ├── icons/                    # Íconos de app
│   ├── Icon-Sentrix-*.png       # Íconos PWA
│   └── _headers                 # Headers de Netlify/Vercel
│
├── src/
...
├── nginx-cache.conf             # Configuración de caché para Nginx
...
La aplicación utiliza un sistema de diseño cohesivo basado en colores institucionales profesionales con soporte completo para modo claro y oscuro.

### Modo Claro (Light Mode)

**Colores Base:**
- `background`: `#FFFFFF` - Fondo blanco limpio
- `foreground`: `#2B2520` - Marrón muy oscuro para texto
- `card`: `#FFFFFF` - Cards con fondo blanco
- `muted`: `#F5F3F0` - Beige muy claro para fondos secundarios
- `border`: `#E5E0DB` - Beige/gris claro para bordes

**Colores Principales:**
- `primary`: `#7D5A3B` - Marrón chocolate (botones principales, enlaces)
- `secondary`: `#5B8FB9` - Azul grisáceo institucional
- `accent`: `#E8B923` - Amarillo mostaza (destacados)
- `destructive`: `#C73E1D` - Rojo terracota (acciones destructivas)

**Colores de Branding Sentrix:**
- `sentrix-chocolate`: `#7D5A3B` - Marrón chocolate
- `sentrix-steel`: `#5B8FB9` - Azul institucional
- `sentrix-mustard`: `#E8B923` - Amarillo mostaza
- `sentrix-darkChocolate`: `#654A30` - Marrón oscuro
- `sentrix-burnedOrange`: `#D97941` - Naranja quemado

**Colores de Status/Riesgo:**
- `status-success`: `#5B8FB9` - Azul (bajo riesgo/success)
- `status-warning`: `#E8B923` - Amarillo mostaza (medio riesgo)
- `status-danger`: `#D97941` - Naranja quemado (alto riesgo)
- `status-critical`: `#C73E1D` - Rojo terracota (crítico)
- `status-info`: `#7D8A8F` - Gris azulado (información)

### Modo Oscuro (Dark Mode)

**Colores Base:**
- `background`: `#0F1418` - Azul oscuro profundo
- `foreground`: `#F5F3F0` - Beige claro para texto
- `card`: `#1A2026` - Azul gris oscuro
- `muted`: `#232B32` - Azul medio oscuro
- `border`: `#2E3A42` - Azul gris para bordes

**Colores Principales:**
- `primary`: `#A9825F` - Marrón cálido claro
- `secondary`: `#7BA8CC` - Azul brillante
- `accent`: `#89B6E3` - Azul celeste
- `destructive`: `#E85D42` - Rojo brillante

**Colores de Status/Riesgo (Dark Mode):**
- `status-success`: `#7BA8CC` - Azul brillante (bajo riesgo)
- `status-warning`: `#F5D049` - Amarillo brillante (medio riesgo)
- `status-danger`: `#F48442` - Naranja luminoso (alto riesgo)
- `status-critical`: `#E85D42` - Rojo brillante (crítico)

### Escalas de Color

Cada color principal tiene una escala completa de 50 a 950 para variaciones:

```typescript
// Ejemplo: primary scale
primary-50   // Muy claro - #FAF8F5
primary-100  // ...
primary-600  // Base light mode - #7D5A3B
primary-500  // Base dark mode - #A9825F
primary-900  // Muy oscuro - #453220
```
...
// Colores Sentrix personalizados
<div className="bg-sentrix-chocolate text-white">Chocolate</div>
<div className="bg-sentrix-steel text-white">Steel Blue</div>

// Status/Risk colors
<Badge className="bg-status-success">Bajo Riesgo</Badge>
<Badge className="bg-status-warning">Medio Riesgo</Badge>
<Badge className="bg-status-danger">Alto Riesgo</Badge>
<Badge className="bg-status-critical">Crítico</Badge>

// Dark mode automático
<div className="bg-background text-foreground">
  Se adapta automáticamente al tema del usuario
</div>
```
