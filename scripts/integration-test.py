...
def test_port_configuration():
    ...
    if expected_yolo_url in backend_settings.external_services.yolo_service_url:
        print("   [OK] Backend → YOLO service URL configuration correct")
    else:
        print(f"   [ERROR] Backend → YOLO service URL mismatch: {backend_settings.external_services.yolo_service_url}")
        return False
...
def test_shared_library_imports():
    ...
    try:
        ...
        print("   [OK] Enum values are consistent")
        ...
        print(f"   [ERROR] Enum value mismatch: {e}")
        return False
    ...
    try:
        ...
        for key in expected_keys:
            if key not in risk_result:
                print(f"   [ERROR] Missing key in risk assessment: {key}")
                return False
    ...
    try:
        ...
        for var in required_vars:
            if var not in env_content:
                print(f"   [ERROR] Missing required variable in .env.example: {var}")
                return False

        print("   [OK] All required environment variables defined")
        ...
        if "8001" in yolo_config.url:
            print("   [OK] Shared config manager uses correct port")
        else:
            print(f"   [ERROR] Shared config manager port mismatch: {yolo_config.url}")
            return False
...
def test_documentation_consistency():
    ...
    try:
        ...
        if not import_doc.exists():
            print("   [ERROR] Import conventions documentation not found")
            return False

        print("   [OK] Import conventions documentation exists")
        ...
        if "Port 8001" in doc_content and "YOLO Service" in doc_content:
            print("   [OK] Documentation mentions correct YOLO service port")
        else:
            print("   [WARNING]  Documentation may not reflect correct ports")

        # Check for shared import examples
        if "from shared.data_models import" in doc_content:
            print("   [OK] Documentation shows correct import pattern")
        else:
            print("   [ERROR] Documentation missing shared import examples")
            return False
...
def test_backend_integration():
    ...
    try:
        ...
        if app.title:
            print(f"   [OK] Backend app initializes: {app.title}")

        return True

    except Exception as e:
        print(f"   [ERROR] Backend integration error: {e}")
        return False
...
def test_yolo_service_integration():
    ...
    try:
        ...
        if hasattr(yolo_server, 'app'):
            print("   [OK] YOLO service app can be imported")

        return True

    except Exception as e:
        print(f"   [ERROR] YOLO service integration error: {e}")
        return False
