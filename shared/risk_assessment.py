...
def get_risk_recommendations(
    ...
    if risk_level == RiskLevel.ALTO:
        recommendations.extend([
            ...
            "📞 Contactar autoridades sanitarias locales",
            "🚫 Evitar acumulación de desechos"
        ])
        if high_risk_count > 3:
            recommendations.append("⚠️ Múltiples focos detectados - considerar intervención a gran escala")

    elif risk_level == RiskLevel.MEDIO:
        recommendations.extend([
            "👀 Monitoreo regular necesario",
            "🧹 Limpiar recipientes y contenedores",
            "📅 Programar inspección de seguimiento",
            "💡 Implementar medidas preventivas"
        ])
        if medium_risk_count > 2:
            recommendations.append("📊 Considerar evaluación más detallada del área")

    elif risk_level == RiskLevel.BAJO:
        recommendations.extend([
            "🔧 Mantenimiento preventivo",
            "🧽 Limpieza regular del área",
            "👁️ Vigilancia periódica",
            "📋 Mantener prácticas de prevención"
        ])

    else:  # MINIMO
        recommendations.extend([
            "✅ Continuar vigilancia rutinaria",
            "🔄 Mantener prácticas preventivas actuales"
        ])

    # Add general recommendations based on detection count
    if total_detections > 10:
        recommendations.append("📈 Alto número de detecciones - considerar intervención integral del área")
    elif total_detections > 5:
        recommendations.append("📊 Múltiples puntos detectados - revisar drenaje y limpieza general")
