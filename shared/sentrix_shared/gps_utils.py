...
def extract_gps_from_exif(image_path: Union[str, Path]) -> Dict[str, Any]:
    ...
    Supports: JPG, PNG, TIFF, HEIC, HEIF, WebP
    """
    try:
    ...
    try:
        ...
        if file_extension in ['.heic', '.heif']:
            # Try to register HEIF plugin
            try:
                from pillow_heif import register_heif_opener
                register_heif_opener()
                # Now open the HEIC/HEIF file as normal
                image = Image.open(image_path)
                exif_data = image.getexif()
            except ImportError:
                # If pillow_heif not available, try alternative method
                try:
                    import pyheif
                    # Convert HEIF to PIL Image
                    ...
                    if not exif_data:
                        result['error'] = 'No EXIF data in HEIC file'
                        return result

                except ImportError:
                    result['error'] = 'HEIC support not installed. Install pillow-heif or pyheif.'
                    return result
        else:
            # Standard image formats
            image = Image.open(image_path)
            exif_data = image.getexif()  # Use getexif() instead of _getexif()
        ...
        # Validate GPS info is iterable (sometimes it's an int/IFD pointer)
        if not hasattr(gps_info, 'items'):
            # Try to get GPS IFD from EXIF
            try:
                gps_ifd = exif_data.get_ifd(0x8825)  # GPS IFD tag
                if gps_ifd:
                    gps_info = gps_ifd
                else:
                    result['error'] = 'GPS data format not supported'
                    return result
        ...
        try:
            ...
            result['error'] = f'Error parsing GPS data: {e}'
            return result
        ...
        if file_extension in ['.heic', '.heif']:
            try:
                ...
                try:
                    ...
                    result['error'] = 'HEIC support not installed'
                    return result
        else:
            image = Image.open(image_path)
            exif_data = image.getexif()  # Use getexif() instead of _getexif()
