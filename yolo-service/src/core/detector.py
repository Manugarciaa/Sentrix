...
# Color palette for different breeding site types (BGR format for OpenCV)
# Paleta de colores para diferentes tipos de criaderos (formato BGR para OpenCV)
BREEDING_SITE_COLORS = {
    'Basura': (0, 140, 255),                    # Naranja brillante - Basura/Residuos
    'Charcos/Cumulo de agua': (255, 100, 0),   # Azul - Agua estancada
    'Huecos': (0, 200, 0),                      # Verde - Huecos en el suelo
    'Calles mal hechas': (0, 0, 255),           # Rojo - Calles deterioradas
}

# Default color for unknown classes
DEFAULT_COLOR = (200, 200, 200)  # Gris
...
def _create_processed_image(source_path, result, output_dir=None):
    """
    Crear imagen procesada con detecciones marcadas en colores distintivos

    Colores por tipo de criadero:
    - Basura: Naranja brillante
    - Charcos/Cumulo de agua: Azul
    - Huecos: Verde
    - Calles mal hechas: Rojo
    ...
    try:
        ...
        if result.masks is not None and result.boxes is not None:
            for i, mask in enumerate(result.masks):
                ...
                # Obtener color específico para este tipo de criadero
                color = BREEDING_SITE_COLORS.get(class_name, DEFAULT_COLOR)
        ...
        logger.debug(f"Saved: {Path(output_path).name}")  # Más conciso
